<?php
$titulo_pagina = 'Dashboard Avanzado - Sistema Meteorológico Profesional - ESPOCH';
...
<style>
    ...
    /* ===== VALIDACIÓN VISUAL DE FILTROS ===== */
    .filter-input.error-required {
        ...
        <aside class="sidebar-filters">
            ...
            <div class="filter-section">
                ...
                <div class="filter-group">
                    ...
                    <span class="filter-error-message" id="errorEstacion">⚠️ Debe seleccionar una estación</span>
                </div>
            </div>
                ...
                <div class="filter-group">
                    <div class="date-range-container">
                        <div class="date-range-col">
                            ...
                            <span class="filter-error-message" id="errorAnio">⚠️ Debe seleccionar un año</span>
                        </div>
                        <div class="date-range-col">
                            ...
                            <span class="filter-error-message" id="errorMes">⚠️ Debe seleccionar un mes</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="charts-section level-L2" id="chartsL2" style="display: none;">
                    ...
                    <div class="charts-row">
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartTendenciaL2"></canvas>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="charts-row charts-row-two-cols">
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartClimogramaL2"></canvas>
                            </div>
                        </div>
                        ...
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartAnalisisL2"></canvas>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="charts-row charts-row-two-cols">
                        ...
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartPrecipitacionesL2"></canvas>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="charts-row">
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartAridezL2"></canvas>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="charts-row">
                        <div class="chart-card level-L2">
                            ...
                            <div class="chart-canvas-wrapper">
                                <canvas id="chartHeladasL2"></canvas>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    async function submitDownloadForm() {
        console.log('📥 Enviando formulario de descarga...');

        // Validar que haya filtros seleccionados (sin alert)
        ...
        if (!estacion || !anio || !mes) {
            console.error('❌ ERROR: Faltan filtros obligatorios');
            closeDownloadModal();
            downloadPDF();
            return;
        }
        ...
        btnText.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Generando...';

        try {
            // Llamada al backend
            const response = await fetch('../controller/CDashboard.php?action=download_data', {
                method: 'POST',
                headers: {
            ...
            const extension = tipoDescarga === 'pdf' ? 'pdf' : 'csv';
            const fecha = new Date().toISOString().split('T')[0];
            a.download = `datos_${currentLevel}_${estacion}_${anio}_${mes}_${fecha}.${extension}`;
            ...
            showDownloadSuccessNotification(a.download);
    ...
    function downloadPDF() {
        ...
        const errorEstacion = document.getElementById('errorEstacion');
        ...
        [selectEstacion, selectAnio, selectMes].forEach(select => {
            select.classList.remove('error-required');
        });
        [errorEstacion, errorAnio, errorMes].forEach(error => {
        ...
        if (!estacion) {
            selectEstacion.classList.add('error-required');
            errorEstacion.classList.add('visible');
            hayErrores = true;
        }

        if (!anio) {
            selectAnio.classList.add('error-required');
            errorAnio.classList.add('visible');
            hayErrores = true;
        }

        if (!mes) {
            selectMes.classList.add('error-required');
        ...
        if (hayErrores) {
            console.log('❌ No se puede descargar: faltan filtros obligatorios');

            // Auto-quitar errores después de 3 segundos
            setTimeout(() => {
                [selectEstacion, selectAnio, selectMes].forEach(select => {
                    select.classList.remove('error-required');
                });
        ...
        console.log('✅ Filtros válidos, abriendo modal de descarga...');
    ...
    async function onEstacionChange() {
        ...
        try {
            ...
            const data = await loadStationCompleteData(estacionId, currentLevel);

            // Actualizar interfaz con los datos recibidos
            updateStationInfo(data.estacion);
            updateStatsDisplay(
                data.estadisticas.total_registros,
                data.estadisticas.estaciones_activas,
                data.estadisticas.ultima_actualizacion
            );

            // Poblar años y meses desde la misma respuesta
            populateYearsFromCache(data.tiempo.anios);
            availableYears = data.tiempo.anios;

            // Guardar meses por año para uso posterior
            window.mesesPorAnio = data.tiempo.meses_por_anio;
    ...
    function populateYearsFromCache(anios) {
    ...
    function onAnioChange() {
        ...
        const meses = window.mesesPorAnio && window.mesesPorAnio[anio]
            ? window.mesesPorAnio[anio]
            : [];

        populateMonthsFromCache(meses);
        showWaitingDataInCharts();
    }
    function populateMonthsFromCache(meses) {
        ...
        meses.forEach(mes => {
            const option = document.createElement('option');
            option.value = mes;
            option.textContent = `${mes.toString().padStart(2, '0')} - ${nombres_meses[mes]}`;
            select.appendChild(option);
        });
    }
    ...
    function updateStationInfo(stationData) {
        ...
        // ✅ CONSTRUIR RUTA CORRECTA DESDE views/
        // BD guarda: "public/img/F_Alao2.png"
        // Necesitamos: "../public/img/F_Alao2.png"

        let rutaFotografia = stationData.imagenes?.fotografia;
        let rutaMapa = stationData.imagenes?.mapa;

        // Agregar ../ al inicio si no lo tiene
        if (rutaFotografia && !rutaFotografia.startsWith('../')) {
            rutaFotografia = '../' + rutaFotografia;
        }

        if (rutaMapa && !rutaMapa.startsWith('../')) {
            rutaMapa = '../' + rutaMapa;
        }

        const tieneFotografia = rutaFotografia &&
            rutaFotografia !== '../public/img/default.png' &&
            rutaFotografia !== '../img/default.png';

        const tieneMapa = rutaMapa &&
            rutaMapa !== '../public/img/default.png' &&
            rutaMapa !== '../img/default.png';

        console.log('🖼️ Rutas finales construidas:');
        console.log('   Fotografía:', rutaFotografia);
        ...
        if (tieneFotografia) {
            ...
            <div class="carousel-slide active" data-slide="0">
                <img loading="lazy" src="${rutaFotografia}" 
                     alt="Fotografía de ${stationData.nombre}" 
                     class="carousel-image"
                     onerror="console.error('❌ Error cargando foto:', this.src); this.style.display='none';">
            </div>
        `;
        }
        ...
        if (tieneMapa) {
            ...
            <div class="carousel-slide ${!tieneFotografia ? 'active' : ''}" data-slide="${tieneFotografia ? '1' : '0'}">
                <img loading="lazy" src="${rutaMapa}" 
                     alt="Mapa de ${stationData.nombre}" 
                     class="carousel-image"
                     onerror="console.error('❌ Error cargando mapa:', this.src); this.style.display='none';">
            </div>
        `;
        }
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        try {
            // 🚀 UNA SOLA PETICIÓN AL INICIAR
            await loadInitialDataUnified();
        ...
        // Observer para notificaciones de Plotly
    ...
    async function loadInitialDataUnified() {
        if (cachedInitialData) {
            console.log('✅ Usando datos iniciales desde caché');
            return cachedInitialData;
        }

        try {
            console.log('🔄 Cargando datos iniciales unificados...');

            const response = await fetch('../controller/CDashboard.php?action=get_initial_data');
            const result = await response.json();

            if (!result.success) {
                throw new Error(result.error || 'Error al cargar datos iniciales');
            }

            cachedInitialData = result.data;

            // Poblar select de estaciones
            populateStationSelectFromCache(cachedInitialData.estaciones);

            // Actualizar estadísticas generales
            const stats = cachedInitialData.estadisticas;
            totalEstacionesActivas = stats.estaciones_activas;
            updateStatsDisplay(0, stats.estaciones_activas, '---');

            console.log('✅ Datos iniciales cargados:', {
                estaciones: cachedInitialData.estaciones.length,
    ...
    function populateStationSelectFromCache(estaciones) {
        const select = document.getElementById('filterEstacion');
        select.innerHTML = '<option value="">Seleccione una estación...</option>';
    ...
    async function loadStationCompleteData(id_estacion, nivel) {
        const cacheKey = `${id_estacion}_${nivel}`;

        if (cachedStationData[cacheKey]) {
            console.log(`✅ Usando datos de estación desde caché: ${cacheKey}`);
            return cachedStationData[cacheKey];
        }

        try {
            console.log(`🔄 Cargando datos completos de estación: ${id_estacion} (${nivel})`);

            const url = `../controller/CDashboard.php?action=get_station_complete&id_estacion=${id_estacion}&nivel=${nivel}`;
            const response = await fetch(url);
            const result = await response.json();

            if (!result.success) {
                throw new Error(result.error || 'Error al cargar estación');
            }

            cachedStationData[cacheKey] = result.data;

            console.log('✅ Datos completos de estación cargados');
            return result.data;

        } catch (error) {
            console.error('❌ Error en carga de estación:', error);
            throw error;
        }
    }
